object Lab1 extends jsy.util.JsyApplication {
  ...
  def abs(n: Double): Double = if (n >= 0) n else (n*(-1))
  ...
  def sqrtErr(c: Double, x0: Double, epsilon: Double): Double = {
  	require(epsilon > 0, epsilon)
  	if(epsilon < abs((x0*x0) - c)) {
  		val vagfrog = sqrtStep(c, x0)
  		sqrtErr(c, vagfrog, epsilon)
  	}
  	else x0
  }
  ...
  def repOk(t: SearchTree): Boolean = {
    def check(t: SearchTree, min: Int, max: Int): Boolean = t match {
      case Empty => true
      case Node(l, d, r) => d < max && d >= min && check(r, d, max) && check(l, min, d)
    }
  ...
  def insert(t: SearchTree, n: Int): SearchTree = t match {
  	...
  	case Node(l, d, r) => {
  		if (n >= d) new Node(l, d, insert(r, n))else new Node(insert(l, n), d, r)
  	}
  }
