...
class ArithmeticSpec extends FlatSpec {
  ...
  "Minus" should "subtract two number values and return a number" in {
    ...
    val d1 = S("0")
    val d2 = S("1")
    val d3 = eval(Binary(Minus, d1, d2))
    assert(d3 === N(-1))
  }
  ...
  "Times" should "multiply two number values and return a number" in {
    ...
    val d1 = S("-1")
    val d2 = S("1")
    val d3 = eval(Binary(Times, d1, d2))
    assert(d3 === N(-1))
  }
...
class ExtraSpec extends FlatSpec {
	"ConstTest" should  "work" in {
		val e1 = eval(ConstDecl("x", If(Binary(Lt, N(5), N(2)), N(10), N(-10)), Binary(Plus, Var("x"), N(10))))
		assert(e1 === N(0))
	}
}
